      ...
      <article className="flex flex-col gap-16">
        {
          works.map((project, index) => (
              <div className='w-full flex flex-col gap-8   sm:flex-row sm:gap-12   2xl:gap-24' key={index}>
                <section className='w-full flex flex-col gap-2 relative   sm:w-1/2 sm:justify-center   lg:gap-4'>
                  <h4 className='font-bold text-2xl text-[#0D0D0D]   lg:text-5xl   2xl:text-6xl'>{project.title}</h4>
                  <p className='font-medium text-sm text-[#595959]   lg:text-lg   2xl:text-xl'>{project.description}</p>
                  <ul className='font-bold text-xs text-[#0D0D0D] flex flex-wrap gap-x-2 gap-y-1   lg:text-sm   2xl:text-lg'>
                    {
                  ...
                  <span className='font-bold text-7xl text-[#0D0D0D] opacity-[.15] absolute bottom-0 right-0   lg:text-9xl   2xl:text-[156px]'>
                    {index < 10 && '0'}{index + 1}
                  </span>
                </section>
