...
export default function Home() {
  return (
    <main id='home' className='h-screen min-h-[700px] w-full flex flex-col items-center justify-center gap-12 relative   lg:flex-row lg:px-24   2xl:px-36'>
      <section className='px-6 w-full flex flex-col items-start justify-start gap-6   lg:px-0 lg:w-3/5   2xl:gap-10'>
        <h1 className='font-bold text-3xl text-[#0D0D0D]   xs:text-4xl   sm:text-5xl   2xl:text-7xl'>front-end developer & graphic designer.</h1>
        ...
        <div className='w-full flex items-center gap-6   lg:gap-10   2xl:gap-16'>
        ...
        <div className='z-20 w-48 h-48 font-bold text-sm text-[#0D0D0D] flex flex-col items-start justify-center absolute   xs:w-52 xs:h-52   lg:w-72 lg:h-72 lg:text-lg   2xl:w-96 2xl:h-96 2xl:text-3xl'>
          {
            technologies.map(tech => (
              <p key={tech} className='opacity-20'>{tech}</p>
            ))
          }
        </div>
      ...
      <footer className='px-6 w-full absolute bottom-0   lg:px-24   2xl:px-36'>
